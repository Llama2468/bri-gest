...
- A "connected hub" (ORCID identity, Cloudflare Workers + D1, a curation-graph data model where the standalone tools emit judgment-events rather than the hub storing papers itself) is specified but not built, and is an explicitly separate project/workstream — see `BACKLOG.md` "The hub — a different project" section before conflating it with the visual redesign above.
